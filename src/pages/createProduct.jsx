...
export default function CreateProduct(){
  ...
  const {adminCreateProduct} = useFunctions()
  ...
  const submitProductRecord = async () => {
    ...
    const params = {
      ...
      "price": productPrice, 
      "quantity": 0, 
      "category_ref_no": selected.id,
      "img_url": fileBase64
    }

    const {response_code} = await adminCreateProduct(params)
    if (response_code === '000'){
      ShowToast("success", "Product was created successfully.")
      setTimeout(() => {
        navigate("/products")
      }, 1000);
    }else{
      ShowToast("error", "Product creation failed. Please try again in a few minutes")
    }
  }
          ...
          <div className="input-wrapper">
            ...
            <div className="mt-2.5 product-p flex flex-col space-y-8">
              ...
              <div className="brand-input-box brand-input-box-alt brand-input-box-alt-1 flex flex-row">
                <textarea id="message" rows="4" onChange={changeProductDescription} class="flex p-2 w-full main-input-box rounded-lg" placeholder="Product Description">{productDescription}</textarea>
              </div>
